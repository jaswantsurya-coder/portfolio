...
export type CardStackItem = {
  ...
  ctaLabel?: string;
  tag?: string;
  skills?: string[]; // Custom property for our skills use case
};
  ...
  return (
    <div className="relative h-full w-full bg-surface">
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-6 md:p-8">
        ...
        <h3 className="text-3xl md:text-4xl font-heading font-semibold tracking-tighter text-white leading-none">
          {item.title}
        </h3>
        {item.description ? (
          <p className="mt-3 text-sm md:text-base text-white/70 font-light leading-relaxed max-w-[95%]">
            {item.description}
          </p>
        ) : null}
        
        {/* Render skills tags if provided */}
        {item.skills && item.skills.length > 0 && (
          <div className="mt-6 flex flex-wrap gap-2">
            {item.skills.map((skill) => (
              <span 
                key={skill} 
                className="px-3 py-1.5 bg-black/40 backdrop-blur-md rounded-full text-[10px] font-mono uppercase tracking-widest text-white/80 border border-white/10 shadow-lg"
              >
                {skill}
              </span>
            ))}
